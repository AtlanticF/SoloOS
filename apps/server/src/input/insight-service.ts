...
import type { Insight, InsightContent, InsightStatus, InsightType, CaptureResult } from '@soloos/shared'
...
function deserialize(row: typeof schema.insights.$inferSelect): Insight {
  return {
    id: row.id,
...
export async function processInputCapture(
  db: DB,
  entry_id: string,
  raw: string,
): Promise<CaptureResult> {
  const now = Math.floor(Date.now() / 1000)
  const fallbackTitle = raw.trim().slice(0, 32) || 'Captured input insight'
  let eventTitle = fallbackTitle

  // Step 1: heuristic extraction
  let { fact, synthesis, vector, certainty, missing } = heuristicParse(raw)

  // Step 2: ask agent for concise title; also use it to fill missing fields if needed
  try {
    const agentConfig = await getAgentConfigRaw(db)
    if (agentConfig) {
      const agentResult = await agentParseInsight(raw, agentConfig)
      if (agentResult) {
        if (agentResult.title) eventTitle = agentResult.title.slice(0, 32)
        if (!fact && agentResult.content.fact) fact = agentResult.content.fact
        if (!synthesis && agentResult.content.synthesis) synthesis = agentResult.content.synthesis
        if (!vector && agentResult.content.vector) vector = agentResult.content.vector
  ...
  const content: InsightContent = { fact, synthesis, vector }
  const type = deriveInsightType(content, raw)
  const outcome = deriveOutcome(missing, certainty)
  const status: InsightStatus = outcome === 'insight_created' ? 'ACTIVE' : 'INBOX'
...
export async function getInsight(db: DB, id: string): Promise<Insight | null> {
