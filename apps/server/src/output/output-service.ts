...
import { suggestAssociationsForNewCommits } from '../associations/association-suggester'
import type {
 ...
 */
export function computeAllocatedCost(
  additions: number,
  deletions: number,
  dayTotalDiff: number,
  dayCommitCount: number,
  dailyBurn: number = DEFAULT_DAILY_BURN,
): string {
...
export async function getOutputSummary(db: DB): Promise<OutputSummary> {
  ...
  for (const row of rows) {
    const s = row.state as OutputLifecycleState
  ...
  let syncStatus: OutputSummary['sync_status'] = 'idle'
  let lastSyncedAt: number | null = null

  for (const s of syncStates) {
    if (s.sync_status === 'running') { syncStatus = 'running'; break }
    if (s.sync_status === 'deferred_rate_limited') syncStatus = 'deferred_rate_limited'
...
export async function listOutputEvents(
  db: DB,
  opts: { cursor?: string; limit?: number; repoId?: number; projectId?: string; state?: string },
): Promise<OutputEventsPage> {
  const limit = Math.min(opts.limit ?? 50, 100)

  const conditions = []
  if (opts.cursor) conditions.push(gt(schema.output_metadata.id, opts.cursor))
  if (opts.repoId) conditions.push(eq(schema.output_metadata.repo_id, opts.repoId))
  if (opts.projectId) conditions.push(eq(schema.output_metadata.project_id, opts.projectId))
  if (opts.state) conditions.push(eq(schema.output_metadata.state, opts.state))
...
export async function getOutputEventByEventId(
  db: DB,
  eventId: string,
  ...
  const rows = await db
    .select()
    .from(schema.output_metadata)
    .where(eq(schema.output_metadata.event_id, eventId))
    .limit(1)

  return rows[0] ? deserializeOutputEvent(rows[0]) : null
}

// ── Repo bindings ─────────────────────────────────────────────────────────────

export async function listRepoBindings(db: DB): Promise<RepoBinding[]> {
  const rows = await db.select().from(schema.github_repo_bindings)
  return rows.map(deserializeBinding)
}

export async function upsertRepoBinding(
  db: DB,
  body: UpsertRepoBindingRequest,
): Promise<RepoBinding> {
...
export async function getSyncStatus(db: DB): Promise<RepoSyncState[]> {
  ...
  return syncRows.map(row => ({
    ...
    status: row.sync_status as RepoSyncState['status'],
...
export async function triggerSync(
  ...
  // Check rate limiting
  const rateLimited = syncRows.find(
    r => r.next_safe_sync_at !== null && r.next_safe_sync_at > now,
  )
  if (rateLimited) {
    return {
      accepted: false,
...
export async function syncOutputCommits(db: DB, repoIds: number[]): Promise<void> {
  const now = Math.floor(Date.now() / 1000)
  ...
  for (const repoId of repoIds) {
    try {
      ...
      const dayDiffMap = new Map<string, { total: number; count: number }>()
      const commitDetails: Array<{ sha: string; committedAt: number; additions: number; deletions: number; filesChanged: number; message: string; author: string }> = []
      ...
      const newOutputMetadataIds: string[] = []
      ...
      for (const commit of commitDetails) {
        ...
        const existing = await db
          ...
          .where(
            and(
              eq(schema.output_metadata.repo_id, repoId),
              eq(schema.output_metadata.commit_sha, commit.sha),
            ),
          )
        ...
        const allocatedCost = computeAllocatedCost(
          commit.additions,
          commit.deletions,
          dayStats.total,
          dayStats.count,
        ...
        const state: OutputLifecycleState = binding.project_id ? 'ACTIVE' : 'DRAFT'
        ...
        await db.insert(schema.events).values({
          id: eventId,
          entry_id: entryId,
          title: commit.message.slice(0, 80),
          pillar: 'OUTPUT',
        ...
        newOutputMetadataIds.push(metaId)
      ...
      // Fire async association suggestions for new commits (only when bound to a project)
      if (binding.project_id && newOutputMetadataIds.length > 0) {
        suggestAssociationsForNewCommits(db, binding.project_id, newOutputMetadataIds).catch(() => {
          // silently ignore — suggestion errors must not affect sync state
        })
      }
 ...
 */
export async function applyValuationUp(db: DB, projectId: string, revenueToDistribute: string): Promise<void> {
  const now = Math.floor(Date.now() / 1000)
  ...
  const activeRows = await db
    ...
    .where(
      and(
        eq(schema.output_metadata.project_id, projectId),
        eq(schema.output_metadata.state, 'ACTIVE'),
      ),
    )
 ...
 */
export async function applyProjectDeleted(db: DB, projectId: string): Promise<void> {
  const now = Math.floor(Date.now() / 1000)
  await db
    .update(schema.output_metadata)
    .set({
    ...
    .where(
      and(
        eq(schema.output_metadata.project_id, projectId),
        sql`${schema.output_metadata.state} IN ('ACTIVE', 'VALUATED')`,
      ),
    )
}
 ...
 */
export async function applyProjectArchived(db: DB, projectId: string): Promise<void> {
  const now = Math.floor(Date.now() / 1000)
  await db
    .update(schema.output_metadata)
...
function deserializeOutputEvent(
  row: typeof schema.output_metadata.$inferSelect,
): OutputEvent {
  return {
    id: row.id,
    ...
    state: row.state as OutputLifecycleState,
...
function deserializeBinding(
  row: typeof schema.github_repo_bindings.$inferSelect,
): RepoBinding {
  return {
    id: row.id,
    repo_id: row.repo_id,
