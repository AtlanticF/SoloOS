...
import {
  getOutputSummary,
  getOutputEventByEventId,
  listOutputEvents,
  listRepoBindings,
  upsertRepoBinding,
  getSyncStatus,
  triggerSync,
} from '../output/output-service'

const OUTPUT_STATES = ['DRAFT', 'ACTIVE', 'VALUATED', 'GHOST', 'ARCHIVED'] as const

export function outputRouter(db: DB) {
  const app = new Hono()
  ...
  app.get('/events', async (c) => {
    ...
    if (state && !OUTPUT_STATES.includes(state as typeof OUTPUT_STATES[number])) {
  ...
  // GET /api/output/events/:eventId
  app.get('/events/:eventId', async (c) => {
    const eventId = c.req.param('eventId')
    const item = await getOutputEventByEventId(db, eventId)
  ...
  app.post('/sync', async (c) => {
    ...
    try {
      ...
      if (msg.includes('GITHUB_TOKEN')) {
        return c.json(
          { code: 'UNAUTHORIZED_GITHUB', message: 'GitHub token missing or invalid.', retryable: false, hint: 'Set GITHUB_TOKEN in your environment.' },
          401,
        )
      }
      return c.json(
        { code: 'INTERNAL_ERROR', message: msg, retryable: true, hint: null },
        500,
      )
    }
  })
  ...
  app.post('/repo-bindings', async (c) => {
    let body: { repo_id?: number; repo_name?: string; repo_full_name?: string; project_id?: string | null }
    try {
