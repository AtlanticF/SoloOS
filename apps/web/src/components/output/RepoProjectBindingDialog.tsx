...
import type { RepoBinding, GithubRepo } from '@/lib/api'

interface Props {
  open: boolean
  onOpenChange: (open: boolean) => void
  projects: Project[]
  existingBinding?: RepoBinding | null
  presetRepo?: GithubRepo | null
}

export function RepoProjectBindingDialog({ open, onOpenChange, projects, existingBinding, presetRepo }: Props) {
  const { t } = useTranslation()
  const queryClient = useQueryClient()
  const isEdit = !!existingBinding

  const [repoId, setRepoId] = useState<string>(presetRepo?.id?.toString() ?? existingBinding?.repo_id?.toString() ?? '')
  ...
  function getDefaultProjectNameFromRepo(fullName: string): string {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-sm font-semibold" style={{ color: '#e4e4e7' }}>
            {isEdit
              ...
              : t('output.bindingDialog.titleCreate')}
          </h2>
          <button
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs mb-1.5" style={{ color: '#71717a' }}>
              {t('output.bindingDialog.repoLabel')}
            </label>
            <input
              value={repoFullName}
              onChange={e => {
                ...
                if (!repoId) setRepoId('0')
              }}
              placeholder={t('output.bindingDialog.repoPlaceholder')}
              disabled={isEdit || isPresetRepo}
            ...
            {!isEdit && !isPresetRepo && (
              <p className="text-xs mt-1" style={{ color: '#3f3f46' }}>
                Enter the full repo name, e.g. <code className="font-mono">matt/soloos</code>
              </p>
            )}
          </div>

          {/* GitHub Repo ID (hidden for new; auto-resolved in real integration) */}
          {!isEdit && !isPresetRepo && (
            <div>
              <label className="block text-xs mb-1.5" style={{ color: '#71717a' }}>
                GitHub Repo ID
              ...
              />
              <p className="text-xs mt-1" style={{ color: '#3f3f46' }}>
                Find it at: <code className="font-mono">https://api.github.com/repos/owner/repo</code>
              </p>
            </div>
          )}
          ...
          <div>
            <label className="block text-xs mb-1.5" style={{ color: '#71717a' }}>
              {t('output.bindingDialog.projectLabel')}
            </label>
            <select
              value={projectId}
              ...
              style={{ background: '#18181b', borderColor: '#27272a', color: '#e4e4e7' }}
            >
              <option value="">{t('output.bindingDialog.projectPlaceholder')}</option>
              {projects.map(p => (
            ...
            {!showCreateProject ? (
              <button
                ...
                className="text-xs mt-2 px-2.5 py-1.5 rounded-md border transition-colors hover:border-zinc-500"
                style={{ background: '#18181b', borderColor: '#27272a', color: '#a1a1aa' }}
              >
                {t('output.bindingDialog.createProject')}
              </button>
            ) : (
              <div className="mt-2 flex items-center gap-2">
                <input
                  ...
                  placeholder={t('output.bindingDialog.newProjectPlaceholder')}
                ...
                <button
                  ...
                  className="text-xs px-2.5 py-2 rounded-md border disabled:opacity-50"
                  style={{ background: '#18181b', borderColor: '#4ade8040', color: '#4ade80' }}
                >
                  {isCreatingProject ? t('output.bindingDialog.creatingProject') : t('output.bindingDialog.confirmCreateProject')}
                </button>
                <button
                  onClick={() => { setShowCreateProject(false); setNewProjectName('') }}
                  className="text-xs px-2.5 py-2 rounded-md border"
                  style={{ background: 'transparent', borderColor: '#27272a', color: '#71717a' }}
                >
          ...
          {/* Forward-only note — always visible when editing */}
          {isEdit && (
            <div
              className="flex items-start gap-2 rounded-lg px-3 py-2.5 text-xs"
              style={{ background: '#78350f15', borderLeft: '3px solid #fbbf2440', color: '#fbbf24' }}
            >
              <Info size={12} className="flex-shrink-0 mt-0.5" />
              {t('output.bindingDialog.forwardOnlyNote')}
            </div>
          )}
        </div>
        ...
        <div className="flex justify-end gap-2 mt-6">
          <button
            onClick={() => onOpenChange(false)}
            className="text-sm px-4 py-2 rounded-lg border transition-colors hover:border-zinc-600"
            style={{ background: 'transparent', borderColor: '#27272a', color: '#71717a' }}
          >
            {t('output.bindingDialog.cancel')}
          </button>
          <button
            ...
            style={{ background: '#18181b', border: '1px solid #4ade8040', color: '#4ade80' }}
          >
            {isPending ? t('output.bindingDialog.saving') : t('output.bindingDialog.save')}
          </button>
        </div>
      </div>
    </div>
