...
export function OutputHeader({ bindings, projects, selectedRepoId, selectedProjectId, }) {
    ...
    const lastSyncedAt = syncStatus?.repos.reduce((max, r) => {
        if (!r.last_synced_at)
            return max;
    ...
    return (_jsxs("div", { className: "flex flex-wrap items-center gap-3", children: [_jsxs("div", { className: "text-sm rounded-lg px-3 py-1.5 border", style: { background: '#18181b', borderColor: '#27272a', color: '#e4e4e7' }, children: [t('output.repoInfoLabel'), ": ", repoDisplay] }), _jsxs("div", { className: "text-sm rounded-lg px-3 py-1.5 border", style: { background: '#18181b', borderColor: '#27272a', color: '#e4e4e7' }, children: [t('output.projectInfoLabel'), ": ", projectDisplay] }), _jsx("div", { className: "flex-1" }), _jsx("span", { className: "text-xs", style: { color: isRateLimited ? '#fbbf24' : '#52525b' }, children: syncSubtext }), _jsxs("button", { onClick: () => sync(), disabled: isSyncing || isAnySyncing || isRateLimited, className: cn('flex items-center gap-2 text-sm px-3 py-1.5 rounded-lg border transition-colors', (isSyncing || isAnySyncing || isRateLimited) ? 'opacity-50 cursor-not-allowed' : 'hover:border-zinc-600'), style: { background: '#18181b', borderColor: '#27272a', color: '#e4e4e7' }, children: [_jsx(RefreshCw, { size: 13, className: cn(isSyncing || isAnySyncing ? 'animate-spin' : '') }), syncLabel] })] }));
}
